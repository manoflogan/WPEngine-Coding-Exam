...
To call this function:
   python wp_engine.py --input '<fully qualified_file_path_to_input_csv_file>' --output '<fully qualified path to output csv>'
"""
import argparse
import csv
import datetime
...
def fetch_account_status(account_id):
    ...
    Args:
        account_id: integer; account id for which the accounts information is to
            be fetched
...
def read_csv_file(input_csv_file_path):
    ...
    with open(input_csv_file_path, 'r', encoding='utf-8') as file_path:
        ...
        for row in itertools.islice(csv_reader, 1, None):
            yield (int(row[0]), row[2],
                   datetime.datetime.strftime(datetime.datetime.strptime(row[-1], '%m/%d/%y'), '%Y-%m-%d'))
